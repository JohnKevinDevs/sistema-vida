# Roadmap curto

## Atual

- Fase 4.3 concluida: associacao inicial de conversas a projetos.

## Proximas fases sugeridas

### Fase 4.4 - Filtrar conversas por projeto

- Mostrar conversas do projeto selecionado.
- Manter opcao de conversas globais.
- Evitar mover conversas antigas automaticamente.

### Fase 4.5 - Auditoria de projetos e conversas

- Auditar criar, renomear, excluir e selecionar projetos.
- Auditar conversas globais e associadas.
- Garantir que exclusao de projeto nao apague historico indevidamente.

### Fase 5.x - Tarefas reais

- Criar tabela de tarefas.
- Criar endpoints basicos.
- Conectar UI de tarefas ao SQLite.
- Associar tarefas a areas, projetos ou datas quando fizer sentido.

### Fases futuras

- Metas persistentes.
- Revisoes diarias e semanais.
- Rotina e estudos persistentes.
- Memoria do Sistema JK.
- Embeddings e memoria vetorial.
- Busca semantica.
- Backup e exportacao.

## Principio do roadmap

Cada fase deve ser pequena, validavel e sem criar complexidade antes da hora.
